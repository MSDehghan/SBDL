...
namespace SBDL {
    ...
    */
    void updateEvents() {
        ...
        if (Core::keystate_size == 0) {
            Core::keystate = SDL_GetKeyboardState(&Core::keystate_size);
            Core::old_keystate = new Uint8[Core::keystate_size];
            std::fill(Core::old_keystate, Core::old_keystate + Core::keystate_size, 0);
        } else {
            delete[] Core::old_keystate;
            Core::old_keystate = new Uint8[Core::keystate_size];
            std::copy(Core::keystate, Core::keystate + Core::keystate_size, Core::old_keystate);
            Core::keystate = SDL_GetKeyboardState(&Core::keystate_size);
        }
